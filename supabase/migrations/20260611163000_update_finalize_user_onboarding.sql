-- Update finalize_user_onboarding to handle privacy visibility switches
...
DECLARE
  ...
  v_tribe_id uuid;
  v_follow_id uuid;
  v_show_meals boolean;
  v_show_workouts boolean;
  v_show_macros boolean;
  v_show_measurements boolean;
...
BEGIN
  ...
  -- Extract privacy show toggles (or derive from is_private if null)
  v_show_meals := COALESCE((payload->>'show_meals_to_public')::boolean, NOT v_is_private);
  v_show_workouts := COALESCE((payload->>'show_workouts_to_public')::boolean, NOT v_is_private);
  v_show_macros := COALESCE((payload->>'show_macros_to_public')::boolean, NOT v_is_private);
  v_show_measurements := COALESCE((payload->>'show_measurements_to_public')::boolean, NOT v_is_private);
  v_show_likes := COALESCE((payload->>'show_likes_to_public')::boolean, NOT v_is_private);
  ...
  SET 
    ...
    show_meals_to_public = v_show_meals,
    show_workouts_to_public = v_show_workouts,
    show_macros_to_public = v_show_macros,
    show_measurements_to_public = v_show_measurements,
    show_likes_to_public = v_show_likes
  WHERE id = v_user_id;
